...
class SMF:
    ...
    def build_graph(self):
        ...
        self.MSE = tf.losses.mean_squared_error(self.r, self.r_hat)
        self.MAE = tf.reduce_mean(tf.abs(tf.subtract(self.r, self.r_hat)))
        self.l2_loss = tf.add(tf.nn.l2_loss(self.U), tf.multiply(self.reg_lambda, tf.nn.l2_loss(self.V)))
        self.reg_loss = tf.add(self.MSE, self.l2_loss)

        self.optimizer = tf.train.AdamOptimizer(self.learning_rate)
        self.train_step_u = self.optimizer.minimize(self.reg_loss, var_list=[self.U])
        self.train_step_v = self.optimizer.minimize(self.reg_loss, var_list=[self.V])
    ...
    def train_test_validation(self, M, train_idx, test_idx, valid_idx, n_steps=100000, result_path='result/'):
        ...
        for step in range(1, n_steps):
            ...
            _, mse, mae, summary_str = self.sess.run(
                [self.train_step_u, self.MSE, self.MAE, self.summary_op], feed_dict=feed_dict)
